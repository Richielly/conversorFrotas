...
from Layout import fornecedorLayout
...
_layout = fornecedorLayout.FornecedorLayout()
_log = log.Log()
_path_file = path_dir + _core.step['Fornecedor'][3]

table_name = _layout.table_name()
# next_id = _validation.last_id(table_name) #Comentado quando não vai gravar este tipo de entidade
...
class FornecedorLayoutData:
    ...
    def exist(self, CODFORNECEDOR):
        try:
            cursor = conn.connection()
            script = f" select * from Fornecedor  where CODFORNECEDOR = {CODFORNECEDOR}"
    ...
    def insert_data(self, values, homologacao=False):
        try:
            ...
            query = f"INSERT INTO Fornecedor ({columns}) VALUES ({value})"
            ...
            _log.log(f'Erro ao tentar inserir o dado: {table_name}')
