...
_layout = saldoAnteriorBemLayout.SaldoAnteriorBemLayout()
_log = log.Log()
_path_file = path_dir + _core.step['SaldoAnteriorBem'][3]
...
class SaldoAnteriorBemLayoutData:
    ...
    def exist(self, CODENTIDADE, EXERCICIO, CODENTIDADE_BEM, CODBEM, CODENTIDADE_CC, EXERCICIO_CC, CODCONTACONTABIL):
        try:
            cursor = conn.connection()
            script = f""" SELECT
                    sab.*
                    ...
                    and sab.EXERCICIO = {EXERCICIO} 
                    and b.CODENTIDADE = {CODENTIDADE_BEM}
                    and b.CODBEM = {CODBEM}
                    and cc.CODENTIDADE = {CODENTIDADE_CC}
                    and cc.EXERCICIO = {EXERCICIO_CC}
                    and cc.CODCONTACONTABIL = '{CODCONTACONTABIL}' """
            ...
            _log.log(msg=f"Ocorreu um erro ao consultar o banco de dados {table_name}: {e}")
    ...
    def insert_data(self, values, homologacao=False):
        try:
            ...
            query = f"INSERT INTO SCP55_SALDOANTERIORBEM ({columns}) VALUES ({value})"
            ...
            _log.log(msg=f"Erro ao tentar inserir o dado tabela {table_name}: {e}")
