...
class CorLayoutData:
    ...
    def save_data(self, table_name, values, constrant, line):
        if (_validation.exist(table_name, values, constrant, True)) == None:
            ...
            _log.log(f'Linha {line}: Cor "{values}" foi gravada com sucesso.', filename=table_name, level=logging.INFO)
            return True
        else:
            _log.log(f'Linha {line}: A cor "{values}", já esta gravada.', filename=table_name, level=logging.WARN)
            return False
...
for line in pd.read_csv(_path_file, chunksize=1, header=None):
    ...
    nmCor = values[0]
    _corLayoutData = CorLayoutData()

    _corLayoutData.save_data(table_name, [nmCor], constrant, line_index)
