...
from Layout import pessoaJuridicaLayout
...
_layout = pessoaJuridicaLayout.PessoaJuridicaLayout()
_log = log.Log()
_path_file = path_dir + _core.step['PessoaJuridica'][3]
...
class PessoaJuridicaLayoutData:
    ...
    def exist(self, CNPJ):
        try:
            cursor = conn.connection()
            script = f" select * from PESSOAJURIDICA where CNPJ = '{CNPJ}' "
    ...
    def insert_data(self, values, homologacao=False):
        try:
            ...
            query = f"INSERT INTO PESSOAJURIDICA ({columns}) VALUES ({value})"
            ...
            _log.log(f'Erro ao tentar inserir o dado na tabela: {table_name}: {e}')
            return f"Erro ao tentar inserir o dado na tabela {table_name}: {e}"
