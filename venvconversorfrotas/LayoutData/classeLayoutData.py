...
from Layout import classeLayout
...
_layout = classeLayout.ClasseLayout()
_log = log.Log()
_path_file = path_dir + _core.step['Classe'][3]
...
class ClasseLayoutData:
    ...
    def exist(self, CODGRUPO, CODSUBGRUPO, NOMECLASSE):
        try:
            cursor = conn.connection()
            script = f""" SELECT * FROM CLASSE
                        WHERE CODGRUPO = {CODGRUPO}
                        and CODSubGRUPO = {CODSUBGRUPO}
                        AND lower(NOME) LIKE '%' || lower('{NOMECLASSE}') || '%'; """
    ...
    def insert_data(self, values, homologacao=False):
        try:
            ...
            query = f"INSERT INTO CLASSE ({columns}) VALUES ({value})"
