...
from Layout import grupoLayout
...
_layout = grupoLayout.GrupoLayout()
_log = log.Log()
_path_file = path_dir + _core.step['Grupo'][3]
...
class GrupoLayoutData:
    ...
    def exist(self, NOME):
        try:
            cursor = conn.connection()
            script = f"SELECT * FROM GRUPO WHERE lower(TRIM(nome)) = lower('{NOME}')"
            print(script)
    ...
    def insert_data(self, values, homologacao=False):
        try:
            ...
            query = f"INSERT INTO GRUPO ({columns}) VALUES ({value})"
