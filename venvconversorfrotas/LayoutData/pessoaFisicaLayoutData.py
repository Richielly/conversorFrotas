...
from Layout import pessoaFisicaLayout
...
_layout = pessoaFisicaLayout.PessoaFisicaLayout()
_log = log.Log()
_path_file = path_dir + _core.step['PessoaFisica'][3]
...
class PessoaFisicaLayoutData:
    ...
    def exist(self, CODPESSOA):
        try:
            cursor = conn.connection()
            script = f" select * from PESSOAFISICA  where "
    ...
    def insert_data(self, values, homologacao=False):
        try:
            ...
            query = f"INSERT INTO PESSOAFISICA ({columns}) VALUES ({value})"
