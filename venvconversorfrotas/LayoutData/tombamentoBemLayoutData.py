...
_layout = tombamentoBemLayout.TombamentoBemLayout()
_log = log.Log()
_path_file = path_dir + _core.step['TombamentoBem'][3]
...
class TombamentoBemLayoutData:
    ...
    def exist(self, CODENTIDADE, CODBEM):
        try:
            cursor = conn.connection()
            script = f" select * from SCP55_TOMBAMENTOBEM  where IDBEM = (select idbem from scp55_bem where codentidade = {CODENTIDADE} and codbem = '{CODBEM}') "
    ...
    def insert_data(self, values, homologacao=False):
        try:
            ...
            query = f"INSERT INTO SCP55_TOMBAMENTOBEM ({columns}) VALUES ({value})"
            ...
            _log.log(msg=f"Erro ao tentar inserir o dado na tabela SCP55_TOMBAMENTOBEM: {e}")
