...
from Layout import subGrupoLayout
...
_layout = subGrupoLayout.SubGrupoLayout()
_log = log.Log()
_path_file = path_dir + _core.step['SubGrupo'][3]
...
class SubGrupoLayoutData:
    ...
    def exist(self, CODGRUPO, NOMESUBGRUPO):
        try:
            cursor = conn.connection()
            script = f""" SELECT * FROM SUBGRUPO
                        WHERE CODGRUPO = {CODGRUPO}
                        AND lower(NOME) LIKE '%' || lower('{NOMESUBGRUPO}') || '%';"""
    ...
    def insert_data(self, values, homologacao=False):
        try:
            ...
            query = f"INSERT INTO SUBGRUPO ({columns}) VALUES ({value})"
